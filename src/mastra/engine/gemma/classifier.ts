...
export interface Classification {
  ...
  lop_be?: string;
  /** Khách hỏi cho TRẺ DƯỚI 16 TUỔI tự đến/đi học một mình, hoặc gửi bé lại mà KHÔNG có người lớn đi kèm. */
  be_khong_nguoi_lon?: boolean;
  /** Khách TỰ KHẲNG ĐỊNH có khuyến mãi/quà tặng/mức giá rẻ hơn (nghe bạn bè, thấy web/nơi khác) như thể chắc chắn có rồi ĐÒI áp dụng — mà bên mình CHƯA xác nhận. */
  khach_khang_dinh_uu_dai?: boolean;
  /** Khách hỏi thẳng mình DƯ/THIẾU bao nhiêu cân so với chuẩn ("vậy em dư mấy cân", "thừa bao nhiêu kg"). */
  khach_hoi_du_can?: boolean;
  /** 2 người (vợ-chồng, 2 bạn) mỗi người MỘT MÔN KHÁC NHAU ("chồng gym, vợ yoga") — không phải cả nhà cùng full. */
  hai_nguoi_khac_mon?: boolean;
}
...
export const CLS_SCHEMA = {
  ...
  properties: {
    ...
    lop_be: { type: "string" },
    be_khong_nguoi_lon: { type: "boolean" },
    khach_khang_dinh_uu_dai: { type: "boolean" },
    khach_hoi_du_can: { type: "boolean" },
    hai_nguoi_khac_mon: { type: "boolean" },
  },
...
/**
 * Schema theo NGỮ CẢNH: khi khách đã ngỏ ý đến mà chưa chốt ngày, `khung_ngay` quyết định hệ
 * thống chào 2 ngày NÀO — bỏ trống là mời nhầm ngày thường cho người hẹn cuối tuần (đo prod
 * 30/07: "cuối tuần này em nhé" → bot mời thứ Sáu). Lúc đó ép model PHẢI trả trường này; các
 * lượt khác vẫn để optional để không tốn token decode mỗi lượt (xem ghi chú "đừng thêm field").
 */
export function clsSchemaFor(
  conv: { wantsCome: boolean; ngayChot: string; closed: boolean },
  /** Tin BOT ngay trước đó có nêu con số tiền không (soi hình thức output của chính bot). */
  botVuaBaoGia = false,
): object {
  const them: string[] = [];
  if (conv.wantsCome && !conv.ngayChot && !conv.closed) them.push("khung_ngay");
  // Ngay sau khi bot báo giá là lúc DUY NHẤT khách chê đắt được — mà `khach_che_dat` là cờ
  // optional nên 12B hay bỏ quên, thành ra lượt phản đối bị xử như lượt bình thường (đo prod
  // 30/07: "đắt thế, chỗ khác có 300k thôi" → không cờ nào bật → bot không neo giá trị, không
  // mời trải nghiệm). Ép model phải TRẢ LỜI RÕ true/false đúng lượt đó.
  if (botVuaBaoGia && !conv.closed) them.push("khach_che_dat");
...
- flow: khách đang cần gì Ở THỜI ĐIỂM NÀY. "fitness" = mọi nhu cầu TẬP (gym/yoga/zumba/bơi/pilates) và mọi MỤC TIÊU của việc tập — giảm cân, tăng cân, tăng cơ, giữ dáng, sức khoẻ, và cả THƯ GIÃN / xả stress. "giai-co" = khách ĐANG ĐAU MỎI cơ-xương-khớp và muốn TRỊ LIỆU cho hết đau (không phải để tập), hoặc hỏi thẳng dịch vụ giải cơ/massage/bấm huyệt. STICKY: tin mới không đổi chủ đề (tin mơ hồ, nối tiếp, cho tên-SĐT-giờ) → GIỮ giá trị trong trạng thái đã biết. Muốn "thư giãn" mà KHÔNG kèm đau = fitness. Không xác định được và trạng thái cũng chưa rõ → "chua-ro".
  ...
  ⛔ ĐAU KỂ KÈM TÌNH TRẠNG SỨC KHOẺ TRONG MẠCH TẬP = KHAI BÁO AN TOÀN, VẪN LÀ "fitness". Người đang hỏi tập (đã nêu bộ môn hoặc mục tiêu giảm cân/tăng cân/sức khoẻ) mà kể thêm "em sau sinh bị đau lưng", "bác lớn tuổi hay đau gối", "em thoát vị nhẹ" là đang cho biết thể trạng ĐỂ TẬP CHO AN TOÀN, KHÔNG phải xin trị liệu → GIỮ "fitness" (đo LIVE 30/07: khách sau sinh muốn giảm mỡ bụng nói thêm "em bị đau lưng nữa" → bị lật sang giai-co → bot báo giá giải cơ Hoa Sen 330 nghìn cho khách của Fami). Chỉ lật sang "giai-co" khi khách nói RÕ muốn được xử lý cho hết đau (hỏi giải cơ/massage/trị liệu, "làm sao cho hết đau", "có chữa được không").
...
- ten_khach / sdt: CHỈ điền khi TIN MỚI cung cấp (vd "Hà 0912000111" → ten_khach "Hà", sdt "0912000111"); không có → "". Tin dạng "Tên + dãy số điện thoại" là khách CUNG CẤP TÊN — chữ đứng trước SĐT là TÊN RIÊNG kể cả khi trùng từ chỉ thời gian: "Mai 0906112233" → ten_khach "Mai" (KHÔNG phải "ngày mai"), "Bảy 09xx" → tên "Bảy"; các tin này ngay_hen/ngay_hen_chuan/gio_hen để "". MỘT KÍNH NGỮ đứng trơ (anh/chị/em/cô/chú/bạn) KHÔNG phải tên → "". ⛔ ĐẠI TỪ khách dùng để tự chỉ mình cũng KHÔNG phải tên: "tư vấn cho tôi khóa học bơi", "báo giá cho mình với", "gửi em bảng giá" → ten_khach BỎ TRỐNG (tuyệt đối KHÔNG lấy "tôi"/"mình"/"em"/"tớ" làm tên — tên đó bị ghi vào phiếu khách và bot sẽ gọi khách sai). Chỉ điền khi khách TỰ GIỚI THIỆU tên ("em tên Hà", "Hà nhé") hoặc gửi tên kèm SĐT.
- doi_tuong: khách TỰ NÓI mình thuộc nhóm có bảng giá riêng: học sinh/sinh viên → "hoc-sinh-sinh-vien"; giáo viên → "giao-vien"; đăng ký cho CẢ NHÀ — từ 2 người nhà TRỞ LÊN CÙNG tập/CÙNG mua trọn gói → "gia-dinh"; công ty/doanh nghiệp → "doanh-nghiep"; không nói gì → "chua-ro".
  ...
  ⛔ Hỏi/đăng ký GIÚP cho DUY NHẤT MỘT người nhà (mẹ/bố/ông/bà/vợ/chồng/1 đứa con) — chỉ 1 người đi tập — KHÔNG phải "gia-dinh" → để "chua-ro" (giữ giá trị cũ nếu có) và báo giá cho ĐÚNG 1 người đó theo môn phù hợp. "gia-dinh" CHỈ khi khách nói RÕ có TỪ 2 người nhà TRỞ LÊN cùng đăng ký. Gán nhầm "gia-dinh" cho 1 người là bot đọc bảng gói Gia đình 2-3 người (12tr/14tr) cho khách chỉ có 1 người → SAI HẲN nhu cầu. VD: "mẹ mình 60 tuổi muốn tập nhẹ", "bố mình đăng ký gym" = 1 người → "chua-ro", KHÔNG phải "gia-dinh".
- bo_mon: bộ môn khách quan tâm, tin mới HOẶC giữ giá trị cũ nếu tin mới không đổi. CHỈ được dùng đúng 7 giá trị: gym/yoga/zumba/boi/pilates/full/eco. Chưa rõ → "".
  ⚠ Khách muốn NHIỀU BỘ MÔN trong cùng một tin thì phân biệt: (a) ĐÚNG 2 môn cụ thể mà KHÔNG có yoga ("chỉ gym với bơi", "gym với zumba", "bơi với zumba thôi", "không tập yoga zumba chỉ gym với bơi") → "eco" (gói Fami ECO 2 môn tự chọn trừ yoga, rẻ hơn thẻ full); (b) 2 môn mà 1 trong 2 LÀ yoga ("gym + yoga", "muốn tập cả bơi và yoga"), HOẶC từ 3 môn trở lên, HOẶC nói chung "tập nhiều môn / cả 4 dịch vụ" → "full". Để nguyên 1 môn khi khách chỉ nhắc 1 môn — báo giá thẻ 1 môn cho người muốn nhiều môn là sai (đo prod 30/07: "gym với zumba" → nhận giá thẻ Gym).
...
- muc_tieu: mục tiêu tập, dạng slug: giam-can/tang-can/tang-co/giu-dang/suc-khoe/thu-gian/hoc-boi. CHỈ điền khi khách NÓI mục tiêu CỦA MÌNH; khách hỏi công dụng của bộ môn ("tập yoga có giảm cân không") KHÔNG phải đổi mục tiêu → giữ giá trị cũ. Chưa rõ → "".
  ⚠ Khách NÓI THẲNG mục tiêu thì PHẢI điền, đừng bỏ trống: "em muốn giảm cân", "muốn giảm mỡ bụng", "muốn xuống vài cân" → "giam-can"; "gầy quá muốn tăng cân", "ăn mãi không lên cân" → "tang-can"; "muốn lên cơ", "tập cho có cơ bắp" → "tang-co"; "tập cho khoẻ" → "suc-khoe". Bỏ trống ở đây là hệ thống neo nhầm gói 1 môn cho người cần lộ trình kết hợp (đo prod 30/07).
...
- the_trang: chiều cao/cân nặng khách vừa nêu, NGUYÊN VĂN — kể cả khi khách chỉ cho MỖI chiều cao ("cao 1m65", "1m65", "mình 1m7") hoặc MỖI cân nặng ("nặng 78kg"); ví dụ đủ cả hai "1m70 55kg". Không có → "" (đừng bịa, đừng suy).
...
- khach_hoi_gia: tin mới có hỏi về giá/học phí/bao nhiêu tiền/gói rẻ hơn/ưu đãi không. Hỏi "có gói nào (cho hai mẹ con / cho nhóm / rẻ hơn...) không" cũng là hỏi giá → true. ⛔ Khách chỉ CHÊ mức giá vừa nghe mà không xin mức khác ("sao đắt thế", "đắt quá", "hơi cao so với anh nghĩ") → false, vì đó là khach_che_dat; lúc đó bot phải nói giá trị chứ không báo thêm số.
  ⛔ Khách CHÊ mức vừa nghe bằng cách SO SÁNH giá nơi khác ("đắt thế, chỗ khác có 300k thôi", "bên kia rẻ hơn") → false (đó là khach_che_dat) — lúc này bot phải nói GIÁ TRỊ + mời trải nghiệm, không đọc lại con số.
...
- gia_hoi_ve: khách đang hỏi giá của NHÓM dịch vụ nào — hệ thống dùng để bơm ĐÚNG mấy dòng bảng giá cần. khach_hoi_gia=false → "". Hỏi giá THẺ TẬP theo tháng — "bao nhiêu tiền 1 tháng", "gói tháng nhiêu tiền", "gói bơi/gym/yoga giá sao", "học phí 1 tháng" → "the-tap" (đây là MẶC ĐỊNH; không chắc thì chọn "the-tap"). Hỏi giá thuê PT kèm riêng/tập cùng HLV theo buổi → "pt-1-1". Hỏi học phí KHOÁ HỌC BƠI (học cho biết bơi, bao nhiêu buổi) → "hoc-boi". ⚠ Khi mạch đang là HỌC BƠI (khách nói "khóa học bơi", "học cho biết bơi", hoặc mục tiêu đang là hoc-boi, hoặc đang hỏi học bơi cho bé/cháu) rồi hỏi giá CHUNG chung ("giá sao vậy", "khóa học bao nhiêu tiền", "học phí thế nào") → "hoc-boi", ĐỪNG rơi về mặc định "the-tap" (báo giá thẻ tháng cho người muốn học bơi là lệch sản phẩm). ⚠⚠ CHỮ "KHÓA HỌC" / "LỚP HỌC" / "HỌC BƠI" trong tin khách LÀ tín hiệu HỌC BƠI — dù là TIN ĐẦU chưa có bộ môn ("khóa học bao nhiêu tiền", "tư vấn khóa học bơi") thì gia_hoi_ve = "hoc-boi", KHÔNG lấy mặc định "the-tap". ⚠⚠ Khi trạng thái đã cho thấy đây là KHÓA HỌC (biết bơi=chua-biet, HOẶC mục tiêu=hoc-boi, HOẶC lượt trước đã báo giá khóa học 1.5tr/3tr) thì các câu hỏi TIẾP về THỜI HẠN ("mấy tháng", "3 tháng", "học bao lâu"), TẦN SUẤT ("mỗi tuần mấy buổi", "tuần học mấy hôm") hay KHUYẾN MÃI/COMBO ("có km gì không", "combo có ưu đãi gì") VẪN giữ "hoc-boi" — TUYỆT ĐỐI ĐỪNG lật sang "the-tap". ⚠⚠ Và câu hỏi giá CHUNG CHUNG ĐẦU TIÊN của mạch đó cũng vậy: khách vừa nói "chưa biết bơi" rồi hỏi "chi phí tập ntn ạ" / "phí thế nào" / "giá sao ạ" / "học phí bao nhiêu" / "bao nhiêu tiền" (KHÔNG nhắc chữ vé/thẻ/tháng nào) → "hoc-boi". Luật "không chắc thì chọn the-tap" KHÔNG áp cho người CHƯA BIẾT BƠI: người chưa biết bơi hỏi chi phí là hỏi HỌC PHÍ KHOÁ HỌC, báo thẻ bơi tháng 700 nghìn cho họ là SAI SẢN PHẨM. Chỉ khi khách nhắc RÕ "vé/thẻ/gói tháng/bơi tự do" mới về "the-tap"/"ve-boi-le". Người/bé CHƯA biết bơi thì sản phẩm đúng là KHOÁ HỌC (12 buổi 1.5tr/3tr), báo THẺ BƠI trẻ em 12 tháng 3.6 triệu cho họ là SAI SẢN PHẨM. ⛔ CỰC KỲ QUAN TRỌNG: trong mạch HỌC BƠI, khi khách hỏi về "1 kèm 1" / "1-1" / "kèm riêng" / "lớp nhóm" / "học nhóm mấy người" thì đó là các HÌNH THỨC của KHOÁ HỌC BƠI → vẫn "hoc-boi" (khoá bơi 1-1 = 3 triệu/12 buổi, lớp nhóm = 1.5 triệu), TUYỆT ĐỐI KHÔNG chọn "pt-1-1" (pt-1-1 là thuê PT tập GYM 20 buổi 6 triệu — báo số đó cho người học bơi là SAI SẢN PHẨM lẫn SAI GIÁ). Chỉ chọn "pt-1-1" khi mạch là tập GYM/tạ và khách hỏi thuê HLV kèm riêng. Hỏi vé bơi TỪNG LƯỢT / bơi TỰ DO theo lượt hoặc theo BUỔI ("bơi 1 lượt bao nhiêu", "vé lẻ", "vé bơi tự do", "vào bơi 1 buổi", "bơi tự do theo buổi tính tiền sao", đã biết bơi chỉ vào bơi theo buổi) → "ve-boi-le" — ⛔ "gói tháng" / "thẻ tháng" KHÔNG phải vé lẻ. Hỏi giá Pilates → "pilates". Hỏi thuê HLV theo GIỜ → "thue-hlv". Bên GIẢI CƠ mà khách hỏi gói NHIỀU BUỔI / lộ trình / liệu trình → "lieu-trinh" (chỉ hỏi giá 1 buổi thì vẫn "the-tap").
- khach_che_dat: khách vừa CHÊ GIÁ CAO sau khi nghe báo giá ("sao đắt thế", "hơi cao", "đắt quá", "hơi đắt so với anh nghĩ"). ⛔ Khách hỏi THẲNG xin lựa chọn rẻ hơn ("có gói nào rẻ hơn không", "gói ngắn hơn thì sao") KHÔNG phải chê đắt → false (đó là hỏi giá, phải báo số cụ thể). Chưa nghe giá mà mới hỏi giá cũng false.
  ⚠ ĐANG MẶC CẢ GIÁ mà khách nói tiếp "không giảm được thật à", "không bớt được à", "sao không giảm cho anh", "giảm chút đi" → đây là đòi BỚT GIÁ TIỀN → khach_che_dat=true (⛔ chữ "giảm" ở đây là giảm GIÁ, TUYỆT ĐỐI đừng hiểu thành mục tiêu giảm cân, đừng đặt muc_tieu="giam-can", đừng bật media giảm cân).
  ⚠ SO SÁNH VỚI NƠI KHÁC CŨNG LÀ CHÊ ĐẮT → khach_che_dat=true VÀ khach_hoi_gia=false: "đắt thế, chỗ khác có 300k thôi", "bên kia rẻ hơn nhiều", "ngoài kia có 250 thôi", "phòng gần nhà em có 300 à". Khách nêu con số của chỗ khác là để CHÊ, không phải xin bên em báo thêm một mức nữa — gán nhầm thành hỏi giá là bot đọc lại đúng con số vừa báo thay vì nói giá trị (đo prod 30/07).
...
- khung_ngay: khách nói KHUNG ngày MƠ HỒ (chưa phải 1 ngày cụ thể): "cuối tuần"/"weekend" → "cuoi-tuan"; "đầu tuần sau" → "dau-tuan-sau"; "tuần sau"/"tuần tới" → "tuan-sau"; "trong tuần"/"ngày thường" → "trong-tuan"; khách nói ngày cụ thể hoặc không nói gì về thời gian → "".
  ⚠ ĐỪNG BỎ SÓT khi khách gắn thêm "này/nay/tới" hay tiểu từ: "cuối tuần này em nhé", "cuối tuần nay", "weekend này", "cuối tuần tới" → VẪN "cuoi-tuan"; "tuần này qua được không" → "trong-tuan". Bỏ trống trường này là hệ thống chào nhầm 2 ngày ĐẦU TUẦN cho người hẹn CUỐI TUẦN (đo prod 30/07: khách nói "cuối tuần này" mà bot mời thứ Sáu).
...
- media: bộ ảnh hệ thống nên GỬI KÈM lượt này, hoặc "none". Ảnh là ĐÒN MỘT LẦN cho mỗi nhóm, bắn sớm 1 nhịp là mất luôn lúc cần nhất → khi lưỡng lự thì chọn "none". Chỉ chọn khác "none" khi tin khách rơi ĐÚNG 1 trong 2 ca:
  ...
  ⛔ PHÂN BIỆT (đây là chỗ hay sai nhất — các câu dưới đều là KỂ TÌNH TRẠNG, KHÔNG phải hoài nghi → "none"):
    ...
    · Khách đang MẶC CẢ GIÁ, xin bớt tiền: "không giảm được thật à", "giảm giá đi", "bớt chút được không", "sao không giảm" → đây là chuyện GIÁ TIỀN, KHÔNG phải hoài nghi kết quả tập → "none" (⛔ TUYỆT ĐỐI đừng gửi ảnh trước-sau giảm cân — chữ "giảm" ở đây là giảm GIÁ, không phải giảm cân).
...
- be_khong_nguoi_lon: true KHI khách hỏi cho một TRẺ DƯỚI 16 TUỔI đến/ở trung tâm mà KHÔNG có người lớn bên cạnh — GỒM CẢ HAI kiểu: (a) bé TỰ ĐI một mình: "bé tự đi có được không", "bố mẹ bận không đưa đi được, cho bé tự đến", "cháu tự đi xe bus tới học bơi được không"; (b) GỬI bé lại nhờ trung tâm TRÔNG thay bố mẹ (bố mẹ KHÔNG ở đó): "gửi bé lại đây học rồi bố mẹ đón sau", "ở trung tâm có ai trông bé giúp không" (khi trước đó đã nói bố mẹ bận/không đưa đón được). CHỈ bật khi rõ là trẻ <16 VÀ ý là đi/ở lại không có người lớn nhà đi kèm. Người lớn tự đi tập, hay bố/mẹ CÓ đưa bé đến và ở lại → BỎ (false).
- khach_khang_dinh_uu_dai: true KHI khách GẮN VÀO CÂU một ưu đãi/khuyến mãi/quà tặng/mức giá cụ thể như thể nó CÓ THẬT (nghe bạn bè, thấy web, gọi điện trước, nơi khác) mà mình CHƯA hề xác nhận — DÙ nói dưới dạng khẳng định hay hỏi-xác-nhận-nhẹ ("...nhỉ", "...đúng không", "...nhé", "...phải không"): "bạn em bảo đăng ký được tặng thêm 3 tháng đúng không", "đăng ký full 1 năm 7 triệu được tặng 3 tháng nhỉ", "vậy có tặng gì không em" (ngay sau khi khách vừa nêu một quà cụ thể), "nghe nói giờ đăng ký năm được tặng quà", "trên web ghi gym có 400k thôi mà", "hôm qua gọi điện bên mình báo có 300k mà", "lấy anh giá 400k như web đi", "chỗ khác mua 1 tặng 1, bên em có không". BẢN CHẤT: khách đưa một mốc/ưu đãi từ NGUỒN NGOÀI rồi muốn mình gật theo — cái bẫy là câu nghe như một câu hỏi bình thường nhưng thực ra đang MỚM cho mình xác nhận một thứ chưa chắc có. Khi nghi ngờ mà câu có nhắc "tặng/khuyến mãi/giảm/giá rẻ hơn + một con số/mốc cụ thể" từ nguồn ngoài → BẬT true. ⛔ ĐỪNG nhầm với: khách chỉ HỎI CHUNG "có khuyến mãi gì không / có tặng gì không" NGAY TỪ ĐẦU khi chưa ai nêu quà cụ thể → false (đó là hỏi thường); khách CHÊ ĐẮT thuần ("đắt thế chỗ khác 300k") mà không đòi áp → khach_che_dat=true.
  ⛔ "giảm" trong ngữ cảnh MẶC CẢ GIÁ = giảm GIÁ tiền, KHÔNG phải giảm CÂN: "không giảm được thật à", "giảm cho anh chút đi", "bớt được không" ngay sau khi bàn giá → đây là khách xin bớt tiền (khach_che_dat hoặc thương lượng), muc_tieu VẪN giữ nguyên, TUYỆT ĐỐI đừng suy ra muc_tieu="giam-can" và đừng bật media giảm cân.
