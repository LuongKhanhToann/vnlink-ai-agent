...
export function buildLogicGate(state: ConversationState, message?: string): string {
  ...
  if (
    ...
    hints.push(
      "[GATE bơi-trẻ-em: KHÔNG pitch gói. Hỏi DEEP từng câu theo flow Fami: " +
        "(1) 'Bên em nhận từ 6 tuổi, bạn nhà mình năm nay mấy tuổi rồi ạ' " +
        "→ (2) test bạo nước: 'Ở nhà bé có dám ngụp nước hay tắm vòi sen không ạ' " +
        "→ (3) sau khi đủ info mới pitch học bơi 1-1 hoặc lớp nhóm.]",
    );
  }
